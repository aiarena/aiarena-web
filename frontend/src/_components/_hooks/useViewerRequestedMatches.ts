...
export const useViewerRequestedMatches = (): ViewerRequestedMatch[] => {
  ...
  return matchNodes.map(node => ({
    ...
    participant1: node.participant1 ? { name: node.participant1.name, id: node.participant1.id } : null,
    participant2: node.participant2 ? { name: node.participant2.name, id: node.participant2.id } : null,
    result: node.result
      ? {
