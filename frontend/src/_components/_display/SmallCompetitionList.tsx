...
export default function SmallCompetitionList({
  competitions,
}: SmallCompetitionListProps) {
  ...
  if (!selectedCompetition) {
    return (
      <div className="text-center text-white">No competitions available.</div>
    );
  }
              ...
              <div key={index}>
                ...
                <div className="overflow-hidden">
                  <table className="w-full table-fixed text-left">
                    ...
                    <tbody>
                      {competition.node.participants.edges.map(
                        (participant, idx) => (
                          <tr
                            key={idx}
                            onClick={() =>
                              handleRowClick(participant.node.bot.name)
                            }
                          ...
                          >
                            ...
                            <td className="px-4 py-2 flex items-center truncate">
                              <Image
                                width={40}
                                height={40}
                                src={getDivisionImage(
                                  participant.node.divisionNum,
                                )}
                            ...
                            <td className="px-4 py-2 truncate">
                              {participant.node.elo}
                            </td>
                            <td className="px-4 py-2 truncate">
                              {participant.node.trend}
                            </td>
                          </tr>
                        ),
                      )}
