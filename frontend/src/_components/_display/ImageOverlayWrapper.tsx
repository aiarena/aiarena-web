    ...
    <div className="relative h-full w-full bg-black pt-0 mt-0 ">
      {sectionDivider ? (
        <div className="relative z-30">
          <SectionDivider
            title={sectionDividerText}
            darken={sectionDividerDarken}
          />
        </div>
      ) : null}
      ...
      <div className={`absolute inset-0 bg-black ${opacityAmount} z-10`}></div>{" "}
      {/* Allow dynamic opacity */}
      <div
        className={`relative z-10 flex items-center justify-center h-full ${paddingY}`}
      >
        {" "}
        {/* Dynamic padding */}
        {children}
