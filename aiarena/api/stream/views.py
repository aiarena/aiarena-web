...
class StreamNextReplayViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        matches = Match.objects.filter(result__isnull=False, requested_by__isnull=True,
                                       round__competition__competitionparticipation__in=CompetitionParticipation.objects.filter(elo__gte=ELO_START_VALUE).exclude(bot__user_id=config.HOUSE_BOTS_USER_ID))
