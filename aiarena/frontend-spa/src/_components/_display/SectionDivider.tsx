...
const SectionDivider = ({ title, darken, className }: SectionDividerProps) => {
  ...
  return (
    <div className={`relative w-full ${className} `}>
      {/* Full-width line */}
      <div
        className={`absolute left-0 w-full h-[2px] shadow shadow-black bg-customGreen  ${bgClass}`}
      ></div>
      ...
      {title ? (
        <div className="relative mx-auto max-w-[40em]">
          ...
          <div
            className={`clip-path-border-trapezoid bg-customGreen ${bgClass} px-6 py-3`}
          >
