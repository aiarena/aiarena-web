...
export default function SimpleToggle({ enabled, onChange }: SimpleToggleProps) {
  return (
    <Switch
      checked={enabled}
      onChange={onChange}
      className={`group relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 ${enabled ? "border-customGreen" : "border-neutral-500"} bg-gray-600 transition-colors duration-200 ease-in-out focus:ring-2 focus:ring-customGreen focus:ring-offset-2 focus:outline-hidden data-checked:bg-customGreen-dark`}
    >
