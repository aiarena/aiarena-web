...
export const useToggleCompetitionParticipation = () => {
  ...
  const [commit, isInFlight] =
    useMutation<useToggleCompetitionParticipationMutation>(graphql`
      mutation useToggleCompetitionParticipationMutation(
        $input: ToggleCompetitionParticipationInput!
      ) {
  ...
  const toggleParticipation: ToggleCompetitionParticipationFunction = (
    bot,
    competition,
    onSuccess,
  ) => {
  ...
  const result: [
    ToggleCompetitionParticipationFunction,
    boolean,
    string | null,
  ] = [toggleParticipation, isInFlight, error];
