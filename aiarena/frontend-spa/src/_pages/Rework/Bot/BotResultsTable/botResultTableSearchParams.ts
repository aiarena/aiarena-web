...
export const BotResultsTableSortingMap: Record<string, string> = {
  ...
  opponent_race: "opponent_plays_race_name",
  result: "result",
  elo_change: "elo_change",
  cause: "result_cause",
  step: "avg_step_time",
  duration: "match__result__game_steps",
  date: "match__result__created",
  tags: "match__tags__tag__name",
};
...
export function encodeFiltersToSearchParams(
  ...
  setOrDelete(
    searchParam,
    "matchType",
    cleanStr(filters.matchType)?.toLowerCase(),
  );
...
export function decodeFiltersFromSearchParams(
  ...
  const competitionId =
    rawCompetitionId && rawCompetitionId.trim() !== ""
  ...
  return {
    ...
    opponentPlaysRaceId: cleanStr(searchParam.get("opponentRace")) as
      | CoreBotRaceLabelChoices
      | undefined,
    opponentPlaysRaceName: cleanStr(searchParam.get("opponentRaceName")) as
      | CoreMatchParticipationResultCauseChoices
      | undefined,

    result:
      (cleanStr(searchParam.get("result"))?.toUpperCase() as
        | CoreMatchParticipationResultChoices
        | undefined) ?? undefined,
    cause:
      (cleanStr(searchParam.get("cause"))?.toUpperCase() as
        | CoreMatchParticipationResultCauseChoices
        | undefined) ?? undefined,
    ...
    matchType:
      (cleanStr(searchParam.get("matchType"))?.toUpperCase() as
    ...
    matchStartedAfter: searchParam.get("after")
      ? cleanStr(searchParam.get("after"))
      : sinceUpdated
        ? botZipUpdated
        : undefined,
    ...
    includeFinished: toBool(
      searchParam.get("finished"),
      DEFAULTS.includeFinished,
    ),
  };
}
